...
export const figlerRoutes = new Elysia({ prefix: "/api/v1/figler" })
   ...
   * Publishes a prompt to the NATS bus for Grapes (now Figler agents) to handle.
   */
  .post("/refactor", async ({ body }) => {
    const { prompt } = body as { prompt: string };
    const nats = await getNatsService();
    
    console.log(`🚀 Figler API: Broadcasting refactor request: "${prompt.substring(0, 50)}..."`);
    ...
    return { 
      success: true, 
      message: "Refactor request broadcast via Figler NATS bus." 
    };
  }, {
    body: t.Object({
      prompt: t.String()
