...
import { X, Send, Sparkles, Bot, User, AlertTriangle, CheckCircle2, TrendingUp, Lightbulb, Shield, Zap, ArrowRight, BarChart3, Lock } from 'lucide-react';
...
export function AILensOverlay({
  ...
  const RiskView = () => <div className="h-full overflow-y-auto p-6">
      ...
      <div className="space-y-3">
        {data.tasks.slice(0, 5).map((task, i) => <div key={task.id} className="flex items-center gap-4 p-3 bg-white border border-slate-100 rounded-lg shadow-sm hover:shadow-md transition-shadow">

            <div className={`w-2 h-12 rounded-full ${i === 0 ? 'bg-rose-500' : i < 3 ? 'bg-amber-500' : 'bg-emerald-500'}`} />
            ...
            <div className="flex-1">
              <div className="flex justify-between mb-1">
                ...
                <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${i === 0 ? 'bg-rose-100 text-rose-700' : i < 3 ? 'bg-amber-100 text-amber-700' : 'bg-emerald-100 text-emerald-700'}`}>

                  {i === 0 ? 'CRITICAL' : i < 3 ? 'WARNING' : 'SAFE'}
                </span>
              </div>
              <p className="text-xs text-slate-500">
                {i === 0 ? 'Blocked by external dependency (Auth API)' : i < 3 ? 'Approaching deadline with no recent activity' : 'On track, normal velocity'}
              </p>
            </div>
          </div>)}
      </div>
    </div>;
  const ProgressView = () => <div className="h-full overflow-y-auto p-6">
      <div className="mb-8">
        <h3 className="font-bold text-slate-900 mb-4 flex items-center gap-2">
        ...
        <div className="h-48 flex items-end gap-2 p-4 bg-slate-50 rounded-xl border border-slate-100 relative">
          {[30, 45, 35, 50, 60, 55, 75].map((h, i) => <div key={i} className="flex-1 flex flex-col justify-end group relative">

              <div className="w-full bg-blue-500 rounded-t-md opacity-80 group-hover:opacity-100 transition-all duration-500" style={{
            height: `${h}%`
          }} />
              ...
              {i === 6 && <div className="absolute -top-8 left-1/2 -translate-x-1/2 bg-blue-600 text-white text-xs font-bold px-2 py-1 rounded shadow-lg">
                  Projected
                </div>}
            </div>)}
          ...
          <div className="absolute inset-0 pointer-events-none opacity-20">
            <svg className="w-full h-full" preserveAspectRatio="none">
              <path d="M0,100 C50,80 100,90 150,70 S250,60 300,40 S400,30 500,20" fill="none" stroke="currentColor" strokeWidth="4" className="text-blue-600" />
      ...
      <div className="grid grid-cols-2 gap-6">
        <div>
          ...
          <div className="space-y-3">
            {['Frontend', 'Backend', 'Design', 'DevOps'].map((area, i) => <div key={area}>
  ...
  const IdeaView = () => <div className="h-full overflow-y-auto p-6 bg-slate-50/50">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        {['Smart Notifications', 'Automated Reporting', 'Voice Commands', 'Dark Mode V2'].map((idea, i) => <motion.div key={idea} initial={{
        opacity: 0,
        y: 20
      }} animate={{
        opacity: 1,
        ...
        delay: i * 0.1
            ...
            <div className="mt-3 pt-3 border-t border-slate-50 flex items-center gap-2">
              <div className="flex -space-x-2">
                {[1, 2].map((k) => <div key={k} className="w-5 h-5 rounded-full bg-slate-200 border border-white" />)}
              </div>
  ...
  const SecurityView = () => <div className="h-full overflow-y-auto p-6">
      <div className="flex items-center justify-center mb-8">
        <div className="relative w-40 h-40 flex items-center justify-center">
          <svg className="w-full h-full transform -rotate-90">
            <circle cx="80" cy="80" r="70" stroke="#e2e8f0" strokeWidth="12" fill="none" />

            <circle cx="80" cy="80" r="70" stroke="#10b981" strokeWidth="12" fill="none" strokeDasharray="440" strokeDashoffset="110" strokeLinecap="round" />
  ...
  const PriorityView = () => <div className="h-full overflow-y-auto p-6">
      ...
      <div className="relative space-y-4 pl-8 before:absolute before:left-3.5 before:top-2 before:bottom-2 before:w-0.5 before:bg-slate-200">
        {data.tasks.slice(0, 4).map((task, i) => <div key={task.id} className="relative bg-white p-3 rounded-lg border border-slate-200 shadow-sm flex items-center gap-3">

            <div className={`absolute -left-8 w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold border-2 border-white ring-2 ${i === 0 ? 'bg-violet-500 text-white ring-violet-100' : 'bg-slate-100 text-slate-500 ring-slate-50'}`}>
            ...
            <div className="flex-1">
              <div className="flex justify-between">
                ...
                {i === 0 && <span className="text-[10px] font-bold bg-violet-100 text-violet-700 px-2 py-0.5 rounded-full">
                    High Impact
                  </span>}
              </div>
  ...
  return <AnimatePresence>
      {isOpen && <>
        ...
        opacity: 0
      }} onClick={onClose} className="fixed inset-0 bg-slate-900/30 backdrop-blur-sm z-40" />
        ...
        y: 20
            ...
            <div className="w-full max-w-4xl h-[80vh] bg-white/95 backdrop-blur-xl rounded-2xl shadow-2xl border border-white/50 overflow-hidden flex flex-col pointer-events-auto">
              {/* Header */}
              <div className={`px-6 py-4 border-b border-slate-100 flex items-center justify-between ${lens.color.replace('text-', 'bg-').replace('600', '50/50')}`}>

                <div className="flex items-center gap-4">
                  <div className={`w-12 h-12 rounded-xl flex items-center justify-center text-3xl shadow-sm bg-white`}>
                ...
                <button onClick={onClose} className="p-2 hover:bg-black/5 rounded-full text-slate-400 hover:text-slate-700 transition-colors">
              ...
              <div className="flex-1 bg-slate-50/30 overflow-hidden relative">
                ...
                {!['risk-scanner', 'progress-analyst', 'idea-generator', 'security-auditor', 'priority-optimizer'].includes(lens.id) && <div className="p-8 text-center text-slate-400">
              ...
              <div className="p-4 bg-white border-t border-slate-100">
                ...
                {messages.length > 0 && <div className="mb-4 max-h-32 overflow-y-auto space-y-2 px-2">
                    {messages.map((msg) => <div key={msg.id} className={`flex gap-2 ${msg.role === 'user' ? 'flex-row-reverse' : ''}`}>

                        <div className={`text-xs px-3 py-2 rounded-lg max-w-[80%] ${msg.role === 'user' ? 'bg-violet-100 text-violet-800' : 'bg-slate-100 text-slate-700'}`}>

                          {msg.content}
                        </div>
                      </div>)}
                    {isTyping && <div className="text-xs text-slate-400 italic px-2">
                        AI is analyzing...
                      </div>}
                  </div>}
                ...
                <div className="relative flex items-center gap-2">
                  ...
                  <input type="text" value={inputValue} onChange={(e) => setInputValue(e.target.value)} onKeyDown={handleKeyDown} placeholder={`Ask ${lens.name} a follow-up question...`} className="flex-1 bg-slate-50 border border-slate-200 rounded-full pl-10 pr-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-violet-500/20 focus:border-violet-500 transition-all" />

                  <button onClick={handleSend} disabled={!inputValue.trim() || isTyping} className="p-3 bg-slate-900 text-white rounded-full hover:bg-slate-800 disabled:opacity-50 disabled:cursor-not-allowed transition-colors shadow-sm hover:shadow-md active:scale-95 transform duration-100">
