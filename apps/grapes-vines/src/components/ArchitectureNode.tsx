...
export function ArchitectureNode({
  ...
  return <motion.div initial={{
    ...
    scale: 1.02
      ...
      {/* Background Gradient */}
      <div className={`absolute inset-0 rounded-2xl bg-gradient-to-br ${style.gradient} to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none`} />


      <div className="relative flex items-start justify-between mb-4">
        <div className={`p-2.5 rounded-xl ${style.iconBg} ${style.iconColor} shadow-sm`}>

          <Icon size={24} />
        </div>
        {port && <span className="text-[10px] font-mono font-bold text-slate-500 bg-slate-50 px-2 py-1 rounded-md border border-slate-100">
            :{port}
          </span>}
      </div>
      ...
      <div className="relative mb-4">
        ...
        {subtitle && <p className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">
            {subtitle}
          </p>}
      ...
      <div className="relative mt-auto pt-4 flex flex-wrap gap-2">
        {tech?.map((t) => <span key={t} className={`px-2 py-1 text-[10px] font-bold uppercase tracking-wide rounded-md border ${style.tagBg} ${style.tagText} ${style.tagBorder}`}>
