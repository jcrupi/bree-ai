...
export function SpecialtyBar({
  ...
  return <div className="w-full bg-white border-b border-violet-100 px-6 py-2 flex items-center gap-6 shadow-sm z-20 relative">
      {/* Label */}
      ...
      <div className="flex items-center gap-2 overflow-x-auto no-scrollbar py-1">
        ...
        return <motion.button key={specialty.id} initial={{
          opacity: 0,
          y: -10
        }} animate={{
          opacity: 1,
          ...
          delay: index * 0.05
        }} onClick={() => onToggleSpecialty(specialty.id)} className={`
              ...
              `} style={{
          backgroundColor: isSelected ? specialty.color : specialty.bgColor,
          color: isSelected ? 'white' : specialty.color,
          borderColor: isSelected ? specialty.color : `${specialty.color}20`,
          boxShadow: isSelected ? `0 4px 12px -2px ${specialty.color}40` : undefined
        }}>
        ...
        {selectedSpecialties.size > 0 && <motion.button initial={{
        opacity: 0,
        scale: 0.9
      }} animate={{
        opacity: 1,
