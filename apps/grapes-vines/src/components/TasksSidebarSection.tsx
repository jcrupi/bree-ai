...
import { LayoutGrid, FolderKanban, Layers, ChevronRight, ChevronDown, CheckCircle2, Circle, Clock, List } from 'lucide-react';
  ...
  return <div className="px-3 py-4 border-b border-violet-50">
      ...
      <div className="flex items-center justify-between px-2 mb-3">
        ...
        <div className="flex bg-violet-50 rounded-lg p-0.5 border border-violet-100">
          <button onClick={() => setGroupingMode('project')} className={`p-1 rounded-md transition-all ${groupingMode === 'project' ? 'bg-white shadow-sm text-violet-600' : 'text-violet-300 hover:text-violet-500'}`} title="Group by Project">

            <FolderKanban size={12} />
          </button>
          <button onClick={() => setGroupingMode('specialty')} className={`p-1 rounded-md transition-all ${groupingMode === 'specialty' ? 'bg-white shadow-sm text-violet-600' : 'text-violet-300 hover:text-violet-500'}`} title="Group by Specialty">
      ...
      <div className="space-y-1">
        {/* All Tasks Button */}
        <button onClick={handleSelectAll} className={`w-full flex items-center gap-3 px-2 py-1.5 rounded-lg text-sm font-medium transition-all ${!selectedProjectId && selectedSpecialties.size === 0 ? 'bg-violet-50 text-violet-900' : 'text-slate-600 hover:bg-violet-50/50 hover:text-violet-700'}`}>

          <div className={`flex items-center justify-center w-5 h-5 rounded-md text-xs shadow-sm ${!selectedProjectId && selectedSpecialties.size === 0 ? 'bg-white text-violet-600' : 'bg-slate-100 text-slate-500'}`}>
      ...
      projects.map((project) => {
        ...
        return <div key={project.id} className="space-y-0.5">
                  <button onClick={() => {
            ...
            onSelectProject(project.id);
                    ...
                    <span className="flex items-center justify-center w-4 h-4 rounded-md text-[10px] shadow-sm" style={{
              ...
              color: project.color
                    ...
                    <span className={`text-[9px] font-bold px-1.5 py-0.5 rounded-full ${isSelected ? 'bg-violet-100 text-violet-600' : 'bg-slate-100 text-slate-500'}`}>
      ...
      Object.values(SPECIALTIES).map((specialty) => {
        const specialtyTasks = tasks.filter((t) => t.specialties?.includes(specialty.id));
        ...
        return <div key={specialty.id} className="space-y-0.5">
                  <button onClick={() => {
