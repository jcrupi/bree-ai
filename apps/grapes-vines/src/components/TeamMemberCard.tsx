...
const categoryStyles: Record<TeamMemberCategory, {
  accentColor: string;
  iconColor: string;
  tagBg: string;
  tagText: string;
  ...
  return <motion.div initial={{
    ...
    x: 4
      ...
      <div className="relative shrink-0">
        <div className={`w-9 h-9 rounded-full flex items-center justify-center text-xs font-bold border ${member.isAI ? 'bg-violet-50 border-violet-100' : 'bg-slate-50 border-slate-200'}`}>

          {member.isAI ? <Bot className={`w-4 h-4 ${style.iconColor}`} /> : <span className="text-slate-600">{member.avatar}</span>}
        </div>
        <div className={`absolute -bottom-0.5 -right-0.5 w-2.5 h-2.5 rounded-full border-2 border-white ${statusColors[member.status]}`} />
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2">
          ...
          {member.isAI && <span className="px-1.5 py-0.5 rounded text-[9px] font-bold bg-slate-100 text-slate-500 uppercase tracking-wider">
              AI
            </span>}
        </div>
      ...
      <div className="hidden sm:flex items-center gap-1.5 justify-end max-w-[40%] flex-wrap">
        {member.skills.slice(0, 3).map((skill) => <span key={skill} className={`px-2 py-0.5 text-[10px] font-medium rounded-full ${style.tagBg} ${style.tagText} whitespace-nowrap`}>
