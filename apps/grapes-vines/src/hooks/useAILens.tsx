...
import { AILens, AILensAnalysis, AgentXResponse, Task, VineConversation, Grape, Project } from '../types';
...
export interface AILensContextType {
  ...
  sendToAgentX: (lensId: string, zoneId: string, data: any) => Promise<AgentXResponse | null>;
  // Registration
...
// ─── Provider ───
export function AILensProvider({
  children
  ...
  const [lastAgentXResponse, setLastAgentXResponse] = useState<AgentXResponse | null>(null);
  const [analysisHistory, setAnalysisHistory] = useState<AILensAnalysis[]>([]);
  const [agentXStatus, setAgentXStatus] = useState<'idle' | 'analyzing' | 'error'>('idle');
  const dropZonesRef = useRef<Map<string, LensDropZone>>(new Map());
  const [dropZones, setDropZones] = useState<Map<string, LensDropZone>>(new Map());
  ...
  // ── AgentX Communication via Bree API Client ──
  const sendToAgentX = useCallback(async (lensId: string, zoneId: string, data: any): Promise<AgentXResponse | null> => {
    ...
    try {
      ...
      const {
        data: response,
        error
      } = await client.api.agentx.analyze({
        lensId,
        targetType: dataType,
        targetId: zoneId,
      ...
      const {
        data: history
      } = await client.api.lenses.byId(lensId).analyses.get();
      if (history) {
  ...
  const handleDrop = useCallback(async (e: React.DragEvent, zone: LensDropZone) => {
    e.preventDefault();
    setIsDragging(false);
    ...
    if (lens) {
      ...
      if (response) {
        ...
        setAnalysisResult(getMockAnalysis(lensId, zone.dataType, zone.getSummary()));
      }
  ...
  return <AILensContext.Provider value={value}>{children}</AILensContext.Provider>;
}
// ─── Hook ───
...
export function useLensDropZone(config: {
  ...
  return {
    ...
    dropClassName: isOver ? 'ring-2 ring-violet-400/50 ring-offset-2 ring-offset-transparent' : isDragging ? 'ring-1 ring-violet-200/30 ring-offset-1 ring-offset-transparent' : ''
  };
}