...
export function useAgentTasks() {
  ...
  useEffect(() => {
    async function fetchAll() {
      setLoading(true);
      const [tasksRes, agentsRes, areasRes, projectsRes] = await Promise.all([client.api.tasks.get(), client.api.agents.get(), client.api.areas.get(), client.api.projects.get()]);
  ...
  const filteredTasks = useMemo(() => {
    ...
    return result;
  }, [tasks, selectedProjectId, selectedAgentId, selectedGrapeId, selectedAreaId, selectedSpecialties]);
  ...
  const addProject = async (project: Omit<Project, 'id' | 'createdAt'>) => {
    const {
      data
    } = await client.api.projects.post(project);
    if (data) {
  ...
  const addTask = async (task: Omit<Task, 'id' | 'createdAt'>) => {
    const {
      data
    } = await client.api.tasks.post(task);
    if (data) {
