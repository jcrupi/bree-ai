...
import { ArrowLeft, CheckCircle2, Circle, Clock, Calendar, MessageCircle, Terminal, Send, Paperclip, Leaf, GitBranch, CheckSquare, ChevronRight, MoreHorizontal, Hash, Command, FileText } from 'lucide-react';
...
function TaskSwitcher({
  ...
  return <div className="flex items-center gap-1 bg-slate-100/50 p-1 rounded-lg">
      {MOCK_TASKS.map((task, index) => {
      const isActive = index === activeIndex;
      return <button key={task.id} onClick={() => onChange(index)} className={`
...
function TabButton({
  ...
  return <button onClick={onClick} className={`
      ...
      <Icon size={16} />
      <span>{label}</span>
      {count !== undefined && count > 0 && <span className={`text-[10px] px-1.5 py-0.5 rounded-full ${active ? 'bg-violet-100 text-violet-700' : 'bg-slate-100 text-slate-500'}`}>
...
export function TaskBoardPage() {
  ...
  const taskZone = useLensDropZone({
    ...
    getSummary: () => `Task "${activeTask.title}" (${activeTask.status}, ${activeTask.priority} priority)`
  });
  ...
  const noteEditor = useCreateBlockNote({
    initialContent: activeTask.agentxNoteContent ? JSON.parse(activeTask.agentxNoteContent) : undefined
  });
  ...
  const noteMetadata = activeTask.agentxNoteId ? getNoteById(activeTask.agentxNoteId) : null;
  return <div className="min-h-screen bg-white text-slate-900 font-sans flex flex-col">
      ...
      <nav className="px-6 py-3 border-b border-slate-100 flex items-center justify-between bg-white sticky top-0 z-30">
        <div className="flex items-center gap-4">
          <Link to="/" className="p-2 -ml-2 hover:bg-slate-50 rounded-full text-slate-400 hover:text-slate-600 transition-colors">
        ...
        <div className="flex items-center gap-4">
          <TaskSwitcher activeIndex={activeTaskIndex} onChange={setActiveTaskIndex} />
      ...
      <main className={`flex-1 max-w-5xl mx-auto w-full p-6 md:p-10 flex flex-col gap-8 ${taskZone.dropClassName} transition-all duration-200`} {...taskZone.dropProps}>
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="flex items-start justify-between gap-4">
            ...
            <div className="flex gap-2 shrink-0">
              <span className={`px-2.5 py-1 rounded-full text-xs font-semibold border ${activeTask.status === 'done' ? 'bg-emerald-50 text-emerald-700 border-emerald-100' : activeTask.status === 'in-progress' ? 'bg-violet-50 text-violet-700 border-violet-100' : 'bg-slate-50 text-slate-600 border-slate-100'}`}>

                {activeTask.status === 'in-progress' ? 'In Progress' : activeTask.status}
              </span>
              <span className={`px-2.5 py-1 rounded-full text-xs font-semibold border ${activeTask.priority === 'urgent' ? 'bg-rose-50 text-rose-700 border-rose-100' : activeTask.priority === 'high' ? 'bg-orange-50 text-orange-700 border-orange-100' : 'bg-slate-50 text-slate-600 border-slate-100'}`}>
        ...
        <div className="border-b border-slate-100 flex gap-6">
          <TabButton active={activeTab === 'task'} onClick={() => setActiveTab('task')} icon={CheckSquare} label="Overview" />

          <TabButton active={activeTab === 'vine'} onClick={() => setActiveTab('vine')} icon={Leaf} label="Vine Chat" count={vineConversation.unreadCount} />

          <TabButton active={activeTab === 'cli'} onClick={() => setActiveTab('cli')} icon={Terminal} label="Terminal" />
        ...
        <div className="min-h-[400px]">
          <AnimatePresence mode="wait">
            {/* TASK VIEW */}
            {activeTab === 'task' && <motion.div key="task" initial={{
            opacity: 0,
            y: 5
          }} animate={{
            opacity: 1,
                ...
                <div className="lg:col-span-2 space-y-8">
                  ...
                  <div>
                    ...
                    <div className="space-y-1">
                      {todos.map((todo) => <button key={todo.id} onClick={() => toggleTodo(todo.id)} className="w-full flex items-start gap-3 p-2 rounded-lg hover:bg-slate-50 transition-colors text-left group">

                          <div className={`mt-0.5 w-4 h-4 rounded border flex items-center justify-center transition-colors ${todo.completed ? 'bg-emerald-500 border-emerald-500' : 'border-slate-300 group-hover:border-violet-400 bg-white'}`}>
                  ...
                  <div className="pt-6 border-t border-slate-100">
                    <div className="flex items-center justify-between mb-4">
                      ...
                      {noteMetadata && <span className="text-xs font-mono text-violet-500 bg-violet-50 px-2 py-0.5 rounded border border-violet-100">
                          {noteMetadata.id}
                        </span>}
                    </div>

                    <div className="bg-slate-50/50 rounded-xl border border-slate-200 overflow-hidden">
                      {activeTask.agentxNoteContent ? <div className="p-4">
                          <BlockNoteView editor={noteEditor} editable={false} theme="light" />

                        </div> : <div className="p-8 text-center text-slate-400 text-sm italic">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="pt-4 border-t border-slate-100">
                      ...
                      <div className="flex -space-x-2">
                        {TEAM_MEMBERS.slice(0, 3).map((m) => <div key={m.id} className="w-8 h-8 rounded-full border-2 border-white bg-slate-100 flex items-center justify-center text-xs font-bold text-slate-600" title={m.name}>
            ...
            {/* VINE CHAT VIEW */}
            {activeTab === 'vine' && <motion.div key="vine" initial={{
            opacity: 0,
            y: 5
          }} animate={{
            opacity: 1,
            ...
            duration: 0.15
                ...
                <div className="flex flex-col items-center justify-center pt-8 pb-6 px-6 relative z-10 bg-white border-b border-slate-50">
                  <div className="w-11 h-11 rounded-[14px] flex items-center justify-center text-white shadow-lg mb-3" style={{
                ...
                boxShadow: '0 4px 16px rgba(45, 106, 79, 0.25)'
                  ...
                  <div className="flex items-center gap-1.5 mt-1">
                    <p className="text-xs font-medium text-[#2d6a4f]">
                      {vineConversation.participants.map((p) => {
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar relative px-8 py-8">
                  ...
                  <div className="absolute inset-0 w-full h-full pointer-events-none overflow-hidden z-0 flex justify-center">
                    <svg width="100%" height="100%" className="opacity-30">
                      <path d="M430,0 Q450,100 430,200 Q410,300 430,400 Q450,500 430,600 Q410,700 430,800 Q450,900 430,1000" fill="none" stroke="#52b788" strokeWidth="2.5" strokeLinecap="round" />
                  ...
                  <div className="relative z-10 flex flex-col max-w-3xl mx-auto pb-20">
                    ...
                    return <motion.div key={msg.id} initial={{
                      ...
                      marginTop
                            ...
                            {/* Avatar */}
                            <div className="w-11 h-11 min-w-[44px] rounded-full flex items-center justify-center text-xs font-vine font-bold text-white shadow-sm z-20 mt-1" style={{
                        ...
                        boxShadow: '0 3px 14px rgba(0,0,0,0.12)'
                            ...
                            {/* Message Body */}
                            <div className={`flex flex-col ${isLeft ? 'items-start' : 'items-end'} min-w-0 flex-1 z-10`}>
                              ...
                              {/* Bubble */}
                              <div className={`relative p-[22px] text-sm leading-relaxed text-[#334155] bg-white transition-shadow hover:shadow-md`} style={{
                          ...
                          boxShadow: isLeft ? '0 2px 12px rgba(45, 106, 79, 0.06)' : '0 2px 12px rgba(2, 132, 199, 0.06)'
                        }}>

                                {/* Accent Strip */}
                                {isLeft ? <div className="absolute top-0 left-0 w-1 h-full rounded-l-[4px] opacity-40" style={{
                            background: 'linear-gradient(to bottom, #52b788, #2d6a4f)'
                          }} /> : <div className="absolute top-0 right-0 w-1 h-full rounded-r-[4px] opacity-35" style={{
                ...
                <div className="p-6 bg-gradient-to-t from-white via-white to-transparent z-20">
                  <div className="relative max-w-2xl mx-auto flex items-center gap-3">
                    <input type="text" value={vineInput} onChange={(e) => setVineInput(e.target.value)} placeholder="Message the team..." className="flex-1 px-6 py-4 rounded-[28px] bg-white text-sm text-[#1e293b] outline-none transition-all placeholder-[#94a3b8]" style={{
                  border: '1.5px solid rgba(45, 106, 79, 0.15)',
                  boxShadow: '0 2px 16px rgba(0,0,0,0.04)'
                }} />

                    <button className="w-12 h-12 rounded-full flex items-center justify-center transition-transform hover:scale-105" style={{
                  background: 'linear-gradient(135deg, #2d6a4f 0%, #52b788 100%)',
                  boxShadow: '0 4px 16px rgba(45, 106, 79, 0.25)'
                }}>
            ...
            {/* CLI VIEW */}
            {activeTab === 'cli' && <motion.div key="cli" initial={{
            opacity: 0,
            y: 5
          }} animate={{
            opacity: 1,
            ...
            duration: 0.15
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar p-4 font-mono text-xs md:text-sm space-y-2">
                  {cliHistory.map((msg, i) => <div key={i} className={`break-words leading-relaxed ${msg.type === 'user' ? 'text-amber-400' : msg.type === 'agent' ? 'text-emerald-400' : msg.type === 'success' ? 'text-green-400' : 'text-slate-500'}`}>
                ...
                <form onSubmit={handleCliSubmit} className="p-3 bg-slate-900 border-t border-slate-800 flex items-center gap-2">

                  <span className="text-emerald-500 font-bold text-sm select-none">
                    {'>'}
                  </span>
                  <input type="text" value={cliInput} onChange={(e) => setCliInput(e.target.value)} className="flex-1 bg-transparent border-none outline-none text-slate-200 font-mono text-sm placeholder:text-slate-700" placeholder="Enter command..." autoComplete="off" autoFocus />
