...
import { ArrowLeft, Command, Activity, ShieldCheck, MessageCircle, Leaf, X } from 'lucide-react';
...
export function VinesPage() {
  ...
  const selectedConversation = VINE_CONVERSATIONS.find((c) => c.id === selectedConversationId);
  // AI Lens drop zone
  ...
  const memberConversations = selectedMemberId ? VINE_CONVERSATIONS.filter((c) => c.participants.includes(selectedMemberId)) : [];
  ...
  const getMessageStyle = (category: TeamMemberCategory) => {
    if (category === 'human-design') {
      return {
        ...
        bubble: 'bg-[#FFF7ED] border border-orange-100 text-slate-800 rounded-tl-none shadow-lg shadow-orange-900/5',
    ...
    return {
      ...
      bubble: 'bg-[#ECFEFF] border border-cyan-100 text-slate-800 rounded-tr-none shadow-lg shadow-cyan-900/5',
      meta: 'text-cyan-500',
      avatar: 'bg-cyan-500 text-white',
      time: 'text-[#8c827d]'
    };
  };
  return <div className="min-h-screen bg-[#1a1614] text-[#e8dcd8] font-sans selection:bg-[#8B4513] selection:text-white overflow-hidden relative flex flex-col">
      {/* Background Texture */}
      ...
      <nav className="relative z-10 px-8 py-6 flex justify-between items-center border-b border-[#8B4513]/20 bg-[#1a1614]/50 backdrop-blur-sm">
        <Link to="/" className="flex items-center gap-2 text-[#8B4513] hover:text-[#A0522D] transition-colors font-medium">
      ...
      <main className={`relative z-10 flex-1 flex overflow-hidden ${vinesPageZone.dropClassName} transition-all duration-200`} {...vinesPageZone.dropProps}>

        {/* Left Side: Visual & Floating Messages */}
        <div className="flex-1 flex flex-col items-center justify-center relative p-8">
          {/* Header (hidden when conversation open) */}
          <AnimatePresence>
            {!selectedConversationId && <motion.div initial={{
            opacity: 0,
            y: -20
          }} animate={{
            opacity: 1,
          ...
          <div className="w-full h-full relative flex items-center justify-center">
            ...
            {/* Vine Visual */}
            <div className={`transition-opacity duration-500 ${selectedConversationId ? 'opacity-40 blur-[2px]' : 'opacity-100'}`}>

              <VinesVisual onSelectMember={handleSelectMember} selectedMemberId={selectedMemberId} onSelectConversation={handleSelectConversation} selectedConversationId={selectedConversationId} />

            </div>

            {/* Floating Messages Overlay */}
            <AnimatePresence>
              {selectedConversationId && selectedConversation && <motion.div initial={{
              opacity: 0
            }} animate={{
              opacity: 1
            }} exit={{
              opacity: 0
                ...
                scale: 1,
                opacity: 1
              }} onClick={() => setSelectedConversationId(null)} className="absolute top-4 right-4 z-50 bg-[#2a2420]/80 backdrop-blur-sm border border-[#8B4513]/30 rounded-full px-4 py-2 text-sm font-bold text-[#8c827d] hover:text-[#e8dcd8] hover:border-[#8B4513]/60 transition-all flex items-center gap-2 shadow-lg">
                  ...
                  return <motion.div key={msg.id} initial={{
                    ...
                    damping: 20
                          ...
                          {/* Left Avatar */}
                          {!isRight && <div className={`w-12 h-12 rounded-full flex items-center justify-center text-sm font-bold shadow-lg flex-shrink-0 mt-1 border-2 border-white/10 ${style.avatar}`}>

                              {getSenderAvatar(msg.senderId)}
                            </div>}

                          <div className={`max-w-[400px] space-y-2`}>
                            <div className={`flex items-baseline gap-3 ${isRight ? 'justify-end' : 'justify-start'}`}>
                              ...
                              minute: '2-digit'
                                  ...
                                  <span className={`text-[11px] font-bold uppercase tracking-wider ${style.meta}`}>
                            ...
                            <div className={`px-6 py-4 text-base leading-relaxed ${style.bubble}`}>
          ...
          {!selectedConversationId && <div className="absolute bottom-8 left-8 right-8 grid grid-cols-3 gap-8 border-t border-[#8B4513]/20 pt-6 max-w-2xl mx-auto">
        ...
        <AnimatePresence mode="wait">
          {selectedMember ? <motion.div key="member-panel" initial={{
          x: '100%',
          opacity: 0
        }} animate={{
          x: 0,
          ...
          stiffness: 200
              ...
              <div className="p-6 border-b border-[#8B4513]/20 bg-[#2a2420]/50">
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center gap-3">
                    <div className="w-12 h-12 rounded-full flex items-center justify-center text-lg font-bold text-white border-2 border-[#8B4513]/50" style={{
                  backgroundColor: selectedMember.category === 'human-design' ? '#D97706' : selectedMember.category === 'human-ai' ? '#0891B2' : '#7C3AED'
                }}>
                  ...
                  <button onClick={() => setSelectedMemberId(null)} className="p-2 hover:bg-[#8B4513]/20 rounded-full text-[#8c827d] transition-colors">

                    <X size={20} />
                  </button>
                </div>
                <div className="flex gap-2 flex-wrap">
                  {selectedMember.skills.map((skill) => <span key={skill} className="px-2 py-1 rounded text-[10px] font-bold uppercase tracking-wide bg-[#8B4513]/10 text-[#A0522D] border border-[#8B4513]/20">
              ...
              {!selectedConversationId && <div className="flex-1 overflow-y-auto custom-scrollbar p-6">
                  <h3 className="text-xs font-bold text-[#5d544f] uppercase tracking-widest mb-4 flex items-center gap-2">
                    <MessageCircle size={12} />
                    Active Conversations
                  </h3>

                  {memberConversations.length === 0 ? <div className="text-center py-12 text-[#5d544f]">
                      <Leaf className="mx-auto mb-2 opacity-20" size={32} />
                      <p>No active vines found.</p>
                    </div> : <div className="space-y-3">
                      {memberConversations.map((vine) => <button key={vine.id} onClick={() => handleSelectConversation(vine.id)} className="w-full text-left p-4 rounded-xl bg-[#2a2420]/50 border border-[#8B4513]/10 hover:border-[#8B4513]/40 hover:bg-[#2a2420] transition-all group">

                          <div className="flex justify-between items-start mb-2">
                            <h4 className="font-bold text-[#e8dcd8] group-hover:text-[#A0522D] transition-colors">
                              {vine.topic}
                            </h4>
                            {vine.unreadCount > 0 && <span className="px-2 py-0.5 rounded-full bg-[#10B981] text-[#1a1614] text-[10px] font-bold">
                                {vine.unreadCount}
                              </span>}
                          </div>
                      ...
                      if (!p) return null;
                      return <div key={pId} className="w-5 h-5 rounded-full border border-[#1a1614] flex items-center justify-center text-[8px] font-bold text-white" style={{
                        backgroundColor: p.category === 'human-design' ? '#D97706' : p.category === 'human-ai' ? '#0891B2' : '#7C3AED'
                      }}>
                            ...
                            <span className="text-[10px] text-[#5d544f]">
                              {new Date(vine.lastActivity).toLocaleDateString(undefined, {
                      month: 'short',
                      day: 'numeric'
                    })}
                            </span>
              ...
              {selectedConversationId && <div className="flex-1 p-8 flex flex-col items-center justify-center text-center opacity-50">
                  <MessageCircle size={48} className="text-[#8B4513] mb-4" />
                  <p className="text-[#8c827d] text-sm">
                    Viewing conversation...
                  </p>
                </div>}
            </motion.div> /* Default Right Panel (System Status) */ : <motion.div key="default-panel" initial={{
          opacity: 0
        }} animate={{
          opacity: 1
        }} exit={{
