...
RuleCatalog:
  ...
  rules:
    ...
    EDC.PROD.012:
      ...
      category: production
      description: "Production must conform to the agreed ESI protocol format with required metadata fields; includes Ringtail/Relativity platform-specific requirements"
      inputs: [esi_protocol_agreed, production_format, metadata_fields_produced, load_files_included, target_platform]
...
**Input:** `esi_protocol_agreed`, `production_format`, `metadata_fields_produced[]`, `load_files_included`, `target_platform` (optional: Relativity, Ringtail, Nuix, Reveal, generic)
**Output:** `production_format_compliant` (PASS | FAIL | INVESTIGATING)

**Minimum Required Metadata (all productions):**
BegBates, EndBates, BegAttach, EndAttach, Custodian, DocDate, Author, Recipients, FileType, NativeFile

**Platform-Specific Additions:**
- **Relativity:** Identifier, Group Identifier, MD5 Hash, Extracted Text (path or inline), Native File Path; delimiters ¶ þ ® ; \
- **Ringtail:** Document_ID (links export/export_extras/pages/parties); flat file: separate Date + Time fields; MDB: export table required
- **Nuix/Reveal:** Same as Relativity; load file .dat/.csv/.txt with header row

**Steps:**
1. If `esi_protocol_agreed` = false → INVESTIGATING ("No agreed ESI protocol on file")
2. Verify `production_format` matches agreed protocol (native, TIFF+OCR, PDF, load file) → mismatch → FAIL
3. Check all minimum required metadata fields are in `metadata_fields_produced` → missing any → FAIL (list missing fields)
4. If `target_platform` specified: verify platform-specific fields present (Identifier/GroupID/MD5 for Relativity; Document_ID for Ringtail) → missing → FAIL
5. If native files required and `load_files_included` = false → FAIL
