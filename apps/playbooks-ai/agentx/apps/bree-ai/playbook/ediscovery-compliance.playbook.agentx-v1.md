...
## Platform-Specific Requirements (Ringtail, Relativity, Nuix, Reveal)

To ensure seamless import into Ringtail, Relativity, Nuix, Reveal, Everlaw, and similar platforms, productions and processing outputs must include the following.

### Relativity (kCura / RelativityOne)

**Supported Load File Formats:** `.dat`, `.csv`, `.txt`, `.opt` (Opticon for images)

**Required / Recommended Fields:**

| Field | Required | Description |
|-------|----------|-------------|
| **Identifier** | Yes | Unique document ID (Control Number / Bates) |
| **Group Identifier** | Recommended | Parent doc ID for email-attachment families |
| **MD5 Hash** | Recommended | Deduplication; documents with same hash = duplicate group |
| **Extracted Text** | Recommended | OCR/full text path or inline text |
| **Native File Path** | Recommended | Path to native files (relative or absolute) |
| **Folder Info** | Optional | Backslash-delimited folder structure (e.g. `Custodian\Email\Inbox`) |
| **Relativity Native Time Zone Offset** | Optional | Decimal offset for email date/time display (e.g. `-5` for EST) |

**Delimiters (for processed data):** Column `¶` (020), Quote `þ` (254), Newline `®` (174), Multi-Value `;` (059), Nested Value `\` (092)

**Date Format:** Single field with date + time (e.g. `6/30/2023 1:23 PM` or `2023-11-28T17:45:39.744-08:00`). Dates cannot be zero.

**Image/OPT Format:** Page-level; each line = one image. Fields: Page ID, Volume ID, Image Path, Document Marker (Y = doc start), Box, Folder, Page Count.

---

### Ringtail (Nuix Discover)

**Supported Load File Formats:** `.dat`, `.csv`, `.txt` (flat files); `.mdb` (Microsoft Access)

**Flat File Structure:**
- First line = field names; subsequent lines = document metadata
- Standard delimiters: Field `¶`, Text qualifier `þ`, Newline `®`, Multi-value `;`
- Date and time must be in **separate fields** (e.g. `Date Modified`, `Time Modified`)
- Document text files: named by Document ID + Control Number; DAT references relative path
- Image reference files (`.OPT` / `.LFP`): line count must match image count

**MDB Load File Tables:**

| Table | Required | Must Have Records | Purpose |
|-------|----------|-------------------|---------|
| **export** | Yes | Yes | Core document metadata; Document_ID is linking field |
| **export_extras** | Yes | No | Additional metadata |
| **pages** | Yes | No | Page-level data |
| **parties** | Yes | No | Custodian/party mapping |
| **annotations** | No | No | Redactions, notes |

**Document_ID** must be present in each document row and links all tables.

---

### Common Production Metadata (ESI Protocol Minimum)

Regardless of platform, productions should include at minimum:

| Field | Description | Use |
|-------|-------------|-----|
| **BegBates** | Start Bates number | Production identification |
| **EndBates** | End Bates number | Production identification |
| **BegAttach** | First attachment Bates | Email families |
| **EndAttach** | Last attachment Bates | Email families |
| **Custodian** | Data source / custodian | Provenance |
| **DocDate** | Document date | Chronology, filtering |
| **Author** | Author / From | Email, authorship |
| **Recipients** | To / CC / BCC | Email distribution |
| **Subject** | Email subject | Search, context |
| **FileType** | Extension (e.g. .msg, .pdf) | Format handling |
| **NativeFile** | Path to native file | Native production |
| **ExtractedText** | OCR/full text path | Search indexing |
| **MD5Hash** | File hash | Deduplication, integrity |
| **GroupID** | Parent document ID | Email-attachment grouping |

---

### Processing Vendor Handoff Checklist

When delivering to Ringtail, Relativity, or similar platforms:

- [ ] Load file format matches platform spec (DAT/CSV/TXT or MDB for Ringtail)
- [ ] Identifier/Control Number field is unique and non-null
- [ ] Group Identifier populated for email-attachment families
- [ ] MD5 hash included for deduplication
- [ ] Extracted text path or inline text provided
- [ ] Native file paths valid and match directory structure
- [ ] Date fields in platform-accepted format (single field for Relativity; separate date/time for Ringtail)
- [ ] Delimiters match platform defaults (¶ þ ® ; \)
- [ ] ZIP structure matches paths in load file
- [ ] OPT/LFP line count matches image count (for image loads)
...
- Produce metadata fields per ESI Protocol (see **Platform-Specific Requirements** for Ringtail, Relativity, Nuix, Reveal)
- **Minimum metadata:** BegBates, EndBates, BegAttach, EndAttach, Custodian, DocDate, Author, Recipients, FileType, NativeFile
- **For Relativity/Ringtail import:** Include Identifier, Group Identifier, MD5 Hash, Extracted Text path, Native File Path; use platform delimiters (¶ þ ® ; \)
