...
const SPECIALTY_CONFIG = [
  ...
  { id: "security-ai",          name: "Security (OWASP)",     icon: "🛡️", catalogId: "bree-ai"   as CatalogId },
  { id: "math-ai",              name: "Math AI",              icon: "🔢", catalogId: "bree-ai"   as CatalogId },
    ...
    <div className="app">
      ...
      <div className="main">
        ...
        <div className={`content-row ${showDocPanel ? "content-row-doc" : ""} ${isPlaybackRunnerTab ? "content-row-playback" : ""} ${isCodeMappingTab ? "content-row-code-mapping" : ""} ${isBuilderTab ? "content-row-builder" : ""} ${isEvolutionTab ? "content-row-evolution" : ""} ${isObserverTab ? "content-row-observer" : ""}`}>
          {/* Left: resizable/collapsible Playbook + Algos panel (when in playbook/algos) */}
          {showDocPanel && !isDesignTab && !isCodeMappingTab && !isBuilderTab && !isEvolutionTab && (
            <div
          ...
          <main className={`doc-panel main-panel ${isPlaybackRunnerTab ? "playback-runner-full" : ""} ${isBuilderTab ? "builder-full" : ""} ${isObserverTab ? "observer-full" : ""} ${showDocPanel ? "main-panel-hidden" : ""}`}>
            {isBuilderTab ? (
              ...
              <PlaybookAnalyzerPanel specialty={specialty} />
...
function PlaybookAnalyzerPanel({ specialty }: { specialty: SpecialtyId }) {
  const [content, setContent] = useState("");
  ...
  const run = async () => {
    ...
    try {
      const base = window.location.origin;
      const isEM = specialty === "enm-ai";
      
      const endpoint = isEM ? `${base}/api/chart-ai` : `${base}/api/generate`;
      
      const inputText = `ANALYSIS REQUEST: What CPT codes do I qualify for based on this chart? Evaluate the documentation against the playbook rules and list all supported codes with rationales.\n\nCHART:\n${content.trim()}`;

      const body = isEM 
        ? { specialty, action: "identify", documentation: content.trim() }
        : { specialty, input_text: inputText };
  ...
  return (
    <div className="playback-runner-panel">
      <div className="playback-runner-form">
        <label className="playback-runner-label">Clinical Chart / Encounter Text</label>
        <textarea
          value={content}
          onChange={(e) => setContent(e.target.value)}
          placeholder="Paste the chart text to identify which codes you qualify for..."
        ...
        <div className="playback-runner-actions">
          <button
            ...
            style={{ 
              padding: "14px 28px", fontSize: 16, height: 54, width: "100%",
              background: "linear-gradient(135deg, #6366f1, #d946ef)",
              boxShadow: "0 4px 12px rgba(99, 102, 241, 0.3)"
            }}
          >
            {loading ? "Analyzing documentation..." : "🔍 Analyze & Detect Qualifying Codes"}
          </button>
        </div>
      </div>
      ...
      {hasResult && !loading && (
        <div style={{ marginTop: 24, display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          {optimization && (
            <div style={{ 
              padding: "16px 20px", borderRadius: 12, background: "#fdf4ff", border: "1.5px solid #f5d0fe",
              color: "#a21caf", fontSize: 14, display: "flex", gap: 12, alignItems: "flex-start"
            }}>
              <span style={{ fontSize: 20 }}>💡</span>
              <div>
                <div style={{ fontWeight: 800, marginBottom: 4, letterSpacing: "0.02em" }}>OPTIMIZATION ADVICE</div>
                {optimization}
              </div>
            </div>
          )}
