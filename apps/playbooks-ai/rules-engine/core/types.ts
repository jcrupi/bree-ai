...
export interface RuleContext {
  encounter: WoundEncounter | EnmEncounter;
  tables: Record<string, unknown>;
  findings: Finding[];
  parsed?: ParsedWoundData | ParsedEnmData;
}

export interface EnmEncounter {
  id?: string;
  ...
  chosen_code?: string;
  modifiers?: string[];
  documented_time_minutes?: number;
  clinical_staff_time_minutes?: number;
  physician_time_minutes?: number;
  primary_icd10?: string;
  secondary_icd10?: string[];
  add_on_codes?: string[];
  payer?: "Medicare" | "other";
  note_text?: string;
  mdm_level?: "straightforward" | "low" | "moderate" | "high";
  claims?: Array<{ code: string; type: "CPT" | "ICD10"; modifiers?: string[] }>;
  patient_history?: Array<{ date: string; specialty: string }>;
}
