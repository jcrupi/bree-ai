...
export function loadPlaybook(specialtyApp: string, baseName: string): DocMeta {
  const spec = SPECIALTY_CONFIG.find(s => s.app === specialtyApp && s.baseName === baseName);
  const playbookDir = spec?.appRoot 
    ? join(APPS_ROOT, spec.appRoot, "playbook")
