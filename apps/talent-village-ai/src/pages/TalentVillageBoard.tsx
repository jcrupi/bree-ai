...
export function TalentVillageBoard() {
  ...
  const [stealthAlerts, setStealthAlerts] = useState<{ id: string; event: string; text: string; ts: string }[]>([]);
  const [stealthDismissed, setStealthDismissed] = useState<Set<string>>(new Set());
    ...
    for (let i = permissionMessages.length - 1; i >= 0; i--) {
      try {
        ...
        if (data.type === 'PERMISSIONS') {
          ...
          setCanObserveLeadTools(!!(data.toolObservers && data.toolObservers.includes(userName)));
          break;
        }
      } catch { /* skip */ }
    }
  ...
  const broadcastPermissions = (chatSet: Set<string>, observerSet: Set<string>) => {
  ...
  useEffect(() => {
    ...
    const handleCopy = () => {
      ...
      sendStealthMessage(userName, JSON.stringify({
        type: 'STEALTH', event: 'copy', text: text.slice(0, 200), ts: new Date().toISOString(),
      })).catch(() => {});
    };
    ...
    const handlePaste = (e: ClipboardEvent) => {
      ...
      sendStealthMessage(userName, JSON.stringify({
        type: 'STEALTH', event: 'paste', text: text.slice(0, 200), ts: new Date().toISOString(),
      })).catch(() => {});
    };
    ...
    const handleCut = () => {
      const text = window.getSelection()?.toString() || '';
      sendStealthMessage(userName, JSON.stringify({
        type: 'STEALTH', event: 'cut', text: text.slice(0, 200), ts: new Date().toISOString(),
      })).catch(() => {});
    };
      ...
      try {
        ...
        if (data.type === 'STEALTH') {
          setStealthAlerts(prev => {
            ...
            return [...prev, { id: msg.id, event: data.event, text: data.text, ts: data.ts }];
          });
        }
      } catch { /* skip non-stealth messages */ }
    });
    ...
    <div className="min-h-screen bg-[#F8FAFC] text-[#1e293b] font-sans overflow-hidden flex flex-col">
      ...
      <header className="bg-white/80 backdrop-blur-md border-b border-slate-200 px-8 py-4 flex items-center justify-between sticky top-0 z-50">
        <div className="flex items-center gap-6">
          <Link to="/setup" className="p-2 hover:bg-slate-100 rounded-full transition-all text-slate-400">
            <ArrowLeft size={20} />
          </Link>
           ...
           {role === 'expert' ? (
             <div className="flex items-center gap-2">
               <div className="flex p-1 bg-slate-100 rounded-full gap-1">
                 {([
                   { id: 'candidate', label: '👤 Candidate', title: 'See candidate view' },
                   { id: 'self', label: isLead ? '👑 Lead Dashboard' : '👤 My Dashboard', title: isLead ? 'My lead dashboard — chat with candidate, advanced tools, invite others' : 'My expert dashboard' },
                   { id: 'experts', label: '👥 Experts', title: 'Expert collaboration' },
                 ] as const).map(tab => (
                   <button
                     key={tab.id}
      ...
      {role === 'expert' && (
        <AnimatePresence>
          {stealthAlerts.filter(a => !stealthDismissed.has(a.id)).map(alert => (
            <motion.div
              key={alert.id}
            ...
            >
              ...
              <span className="uppercase tracking-widest text-[9px] font-black opacity-80">
                {alert.event === 'copy' ? '⚠️ COPY' : alert.event === 'paste' ? '📋 PASTE' : '✂️ CUT'} DETECTED
              </span>
              {alert.text && (
              ...
              <button
                onClick={() => setStealthDismissed(prev => new Set([...prev, alert.id]))}
                  ...
                  <div className="flex-1 bg-white rounded-[32px] border border-slate-200 shadow-xl shadow-indigo-500/5 flex flex-col overflow-hidden min-h-[500px]">
                    <div className="px-6 py-5 bg-[#D3CFEF] border-b border-indigo-200 flex justify-between items-center">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <h4 className="font-bold text-indigo-950 text-sm">Expert Collaboration</h4>
                  ...
                  <div className="flex-1 bg-[#D3CFEF]/30 rounded-[32px] border-2 border-[#D3CFEF] flex flex-col overflow-hidden relative">
                    <div className="px-6 py-4 flex items-center justify-between border-b border-indigo-200/50 bg-[#D3CFEF]/50 flex-shrink-0">
                      <div className="flex items-center gap-3">
                        ...
                        <span className="text-xs font-bold text-indigo-900/60 uppercase tracking-widest">Candidate Mirror • Live</span>
                      </div>
                    ...
                    <div className="flex-1 overflow-y-auto p-6 space-y-4">
                      {assessmentMessages.length === 0 && (
                        <div className="h-full flex flex-col items-center justify-center text-center opacity-30">
                          ...
                          <p className="text-xs font-bold text-indigo-400">Waiting for candidate...</p>
                        </div>
                      )}
                      {assessmentMessages.map((msg) => {
                        const isMe = msg.sender === userName;
                      ...
                      {!isExpertChatCollapsed ? (
                        <>
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <h4 className="font-bold text-indigo-950 text-sm">Expert Vine Chat</h4>
                              <p className="text-[9px] text-indigo-500 font-bold uppercase tracking-widest">
                                {isLead ? 'Lead & Team' : 'Expert Team'} · {isExpertConnected ? 'Live' : 'Connecting...'}
                              </p>
                            </div>
                          </div>
                          <div className="flex items-center gap-2">
              ...
              {!isLead && canObserveLeadTools && (
                <div className="flex flex-col gap-4 mt-6">
                  <div className="flex items-center gap-3 px-2">
                    ...
                    <div>
                      <h4 className="font-bold text-slate-800 text-sm">Lead Tools — Observer View</h4>
                      <p className="text-[9px] text-purple-400 font-bold uppercase tracking-widest">Read-only · Live Data</p>
                    </div>
                      ...
                      <div className="space-y-3 max-h-[260px] overflow-y-auto pr-1">
                        {assessmentQueue.length === 0 ? (
                          <div className="py-8 text-center border-2 border-dashed border-slate-100 rounded-2xl">
                            <p className="text-[10px] font-bold text-slate-300 uppercase tracking-widest">No pending items</p>
                          </div>
                        ) : (
                          assessmentQueue.map((item) => (
                            ...
                            >
                              <div className="flex justify-between items-start gap-3 mb-2">
                                <span className={`text-[9px] font-black uppercase tracking-tighter px-1.5 py-0.5 rounded ${item.isAI ? 'bg-indigo-500 text-white' : 'bg-slate-200 text-slate-500'}`}>
                      ...
                      <div className="space-y-3 pointer-events-none select-none">
                        <div className="flex items-center justify-between p-3 bg-slate-50 rounded-2xl">
                          <div className="flex flex-col">
                            ...
                            <span className="text-[9px] text-slate-400">Lead's setting</span>
                          </div>
                          <div className={`w-10 h-5 rounded-full relative ${isAutoAIEnabled ? 'bg-blue-500' : 'bg-slate-300'}`}>
                        ...
                        <div className="flex items-center justify-between p-3 bg-slate-50 rounded-2xl">
                          ...
                          <div className={`w-10 h-5 rounded-full relative ${isAutoSuggestEnabled ? 'bg-indigo-500' : 'bg-slate-300'}`}>
                            <div className={`absolute top-1 w-3 h-3 bg-white rounded-full transition-all ${isAutoSuggestEnabled ? 'left-6' : 'left-1'}`} />
                          </div>
                        </div>
                        <div className="flex items-center justify-between p-3 bg-slate-50 rounded-2xl">
                          <div className="flex flex-col">
                            <span className="text-[11px] font-bold text-slate-700">Candidate Simulation</span>
                            <span className="text-[9px] text-slate-400">Lead's setting</span>
                          </div>
                          <div className={`w-10 h-5 rounded-full relative ${isActingAsCandidate ? 'bg-emerald-500' : 'bg-slate-300'}`}>
                        ...
                        <div className="flex flex-col gap-2">
                          ...
                          <div className="w-full h-1.5 bg-slate-100 rounded-full">
                            <div className="h-1.5 bg-blue-400 rounded-full" style={{ width: `${(currentQuestionCount / maxQuestions) * 100}%` }} />
                          </div>
                        </div>
                      </div>
                      <p className="text-[9px] text-purple-400 font-bold text-center uppercase tracking-widest mt-4">Observer — Controls Disabled</p>
                    </div>
                    ...
                    <div className="bg-white rounded-[32px] border-2 border-purple-100 p-6 shadow-xl shadow-purple-500/5 flex flex-col">
                      ...
                      <div className="flex-1 flex flex-col gap-3 pointer-events-none select-none">
                        <div className="flex gap-1 text-[9px] font-bold text-slate-500 uppercase tracking-wider px-1">
                          <span className="bg-slate-100 rounded px-2 py-1">{specialty}</span>
                          <span className="bg-slate-100 rounded px-2 py-1">{difficulty}</span>
                          {seed && <span className="bg-slate-100 rounded px-2 py-1 truncate">{seed}</span>}
                        </div>
                        {generatedQuestion ? (
                          <div className="flex-1 p-4 bg-purple-50 rounded-2xl border border-purple-100">
                            <p className="text-[9px] font-black text-purple-400 uppercase tracking-widest mb-2">Latest Generated Question</p>
                            <p className="text-xs text-purple-900 leading-relaxed italic">"{generatedQuestion}"</p>
                          </div>
                        ) : (
                          <div className="flex-1 flex items-center justify-center border-2 border-dashed border-slate-100 rounded-2xl">
                            <p className="text-[10px] font-bold text-slate-300 uppercase tracking-widest text-center p-4">No question generated yet</p>
                          </div>
                        )}
                      </div>
                      <p className="text-[9px] text-purple-400 font-bold text-center uppercase tracking-widest mt-4">Observer — Controls Disabled</p>
                    ...
                    <div className="space-y-4">
                      ...
                      <div>
                        ...
                        <div className="flex gap-1 p-1 bg-slate-100 rounded-xl">
                          {(['beginner', 'junior', 'expert'] as const).map(lvl => (
                            <button key={lvl} onClick={() => setDifficulty(lvl)} className={`flex-1 py-1.5 rounded-lg text-[9px] font-bold uppercase transition-all ${difficulty === lvl ? 'bg-white text-purple-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}>
                              {lvl}
                            </button>
                          ))}
                        </div>
                      ...
                      {generatedQuestion && (
                        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="mt-2 p-4 bg-purple-50 rounded-2xl border border-purple-100">
                          <p className="text-[9px] font-black text-purple-400 uppercase tracking-widest mb-2">Edit before sending</p>
                          <textarea
            ...
            >
              ...
              <div className="px-6 py-5 border-b border-slate-100 flex items-center justify-between bg-gradient-to-r from-emerald-50 to-white">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h2 className="font-bold text-slate-800 text-sm">Expert Roster</h2>
                    <p className="text-[9px] text-slate-400 font-bold uppercase tracking-widest">Access & Observer Controls</p>
                  </div>
                </div>
                <button
                  onClick={() => setIsRosterOpen(false)}
              ...
              <div className="px-6 py-3 flex items-center gap-2 border-b border-slate-50">
                <span className="text-[9px] font-black uppercase tracking-widest text-slate-400">Connected Experts</span>
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-3">
                {expertRoster.length === 0 ? (
                  <div className="h-full flex flex-col items-center justify-center text-center py-16 opacity-50">
                    ...
                    <p className="text-[11px] font-bold text-slate-400 uppercase tracking-widest">No experts yet</p>
                    <p className="text-[10px] text-slate-300 mt-2 max-w-[200px]">
                      Experts appear here after sending a message in the Expert Vine Chat
                    </p>
                  </div>
                ) : (
                  expertRoster.map(name => {
                    ...
                    return (
                      <div key={name} className="bg-slate-50 rounded-2xl border border-slate-100 p-4 space-y-3">
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="flex gap-1 mt-0.5 flex-wrap">
                              {chatEnabled && (
                                <span className="px-1.5 py-0.5 bg-emerald-100 text-emerald-600 text-[8px] font-black rounded uppercase tracking-tighter">Chat ✓</span>
                              )}
                              {isObserver && (
                                <span className="px-1.5 py-0.5 bg-purple-100 text-purple-600 text-[8px] font-black rounded uppercase tracking-tighter">Observer ✓</span>
                              )}
                              {!chatEnabled && !isObserver && (
                                <span className="px-1.5 py-0.5 bg-slate-100 text-slate-400 text-[8px] font-black rounded uppercase tracking-tighter">Read-only</span>
                              )}
                            </div>
                          </div>
                        </div>
                        ...
                        <div className="flex items-center justify-between">
                          <div>
                            <p className="text-[10px] font-bold text-slate-600">Chat with Candidate</p>
                            <p className="text-[9px] text-slate-400">Send messages to the assessment vine</p>
                          </div>
                          <button
                        ...
                        <div className="flex items-center justify-between">
                          <div>
                            ...
                            <p className="text-[9px] text-slate-400">Read-only view of Lead tools</p>
                          </div>
                          <button
              ...
              <div className="px-6 py-4 border-t border-slate-100 bg-slate-50">
                <button
                  onClick={() => { setShowCreateVine(true); setCreatedVineLink(''); setInviteName(''); setInviteRole('expert'); setIsRosterOpen(false); }}
