...
| **domain.agentx.md** | Outlines the domain model: entities, value objects, relationships, and interactions with principals (users, roles, statuses). Defines *what* the app manages. |
...
Use this to:
- Validate that domain entities align with business concepts
- Name entities and value objects in domain language
...
1. **Entities** — Core domain objects with identity (Company, User, Position, Assessment, Message)
2. **Value objects** — Embedded types without identity (JdParsed, CultureData, Question, FeedbackData, ResumeParsed)
3. **Relationships** — Entity associations (Company has Users, Position has Assessments)
...
6. **Interactions** — How principals interact with entities (Recruiter creates Position; Candidate completes Assessment; AI produces FeedbackData). Use to define agent boundaries and API surface.
7. **Field semantics** — Types, optionality, constraints (e.g. scoring_weights sum to 100)

Use this to:
- Define Zod schemas for each entity and value object
...
| **domain-classes.agentx.md** | Full Zod schema spec: enums, value objects, entities with `z.infer` types |
| **domain-postgres.agentx.md** | PostgreSQL tables: entity → table mapping, JSONB for value objects |
...
| `ValueObject` (embedded) | Reference `XSchema` |
| `ValueObject[]` | `z.array(XSchema)` |
...
1. **Identify output type** — Which value object does the agent produce? (e.g. JdParsed, CultureData)
2. **Add validation** — Agent output must be validated with `XSchema.parse()` or `.safeParse()` before use
3. **Define contract** — Input types (domain entities) and output type (value object) in domain-agents.agentx.md
...
- [ ] All entities from domain.agentx have Zod schemas
- [ ] All value objects have Zod schemas
- [ ] All status/role enums use `z.enum([...])`
- [ ] Entity names and field names match domain language from business.agentx
- [ ] Relationships (FKs) are represented correctly
- [ ] Optional fields use `.optional()`
- [ ] PostgreSQL JSONB columns match value object schemas
...
Input: domain.agentx.md
  → Entity: Assessment (position_id, candidate_email, status, scores, feedback_data)
  → Value object: FeedbackData (overall_summary, skill_breakdown, ai_recommendation)
  → Value object: ScoringWeights (technical, cultural, experience, market — sum 100)
